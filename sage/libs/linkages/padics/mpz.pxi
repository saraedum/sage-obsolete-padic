"""
This linkage file implements the padics API using MPIR mpz_t multiprecision integers.
...
cdef inline int ccmp(mpz_t a, mpz_t b, long prec, bint reduce_a, bint reduce_b, PowComputer_class prime_pow) except -2:
    ...
    if reduce_a or reduce_b:
        mpz_init(diff)
        mpz_sub(diff, a, b)
        mpz_mod(diff, diff, prime_pow.pow_mpz_t_tmp(prec))
        mpz_clear(diff)
        return mpz_sgn(diff)
    else:
        return mpz_cmp(a, b)
...
cdef inline int cshift(mpz_t out, mpz_t a, long n, long prec, PowComputer_class prime_pow, bint reduce_afterward) except -1:
    ...
    - ``out`` -- an ``mpz_t`` to store the result.  If `n >= 0` then out will be set to `a * p^n`.
...
cdef inline int cshift0(mpz_t out, mpz_t a, long n, long prec, PowComputer_class prime_pow) except -1:
    """
...
cdef clist(mpz_t a, long prec, bint pos, PowComputer_class prime_pow):
    ...
    if pos:
        curpower = prec
        while mpz_sgn(tmp) != 0 and curpower >= 0:
            list_elt = PY_NEW(Integer)
            mpz_mod(list_elt.value, tmp, prime_pow.prime.value)
            mpz_sub(tmp, tmp, list_elt.value)
            mpz_divexact(tmp, tmp, prime_pow.prime.value)
        ...
        curpower = prec
        mpz_init(halfp)
        mpz_fdiv_q_2exp(halfp, prime_pow.prime.value, 1)
        while mpz_sgn(tmp) != 0 and curpower > 0:
            curpower -= 1
            list_elt = PY_NEW(Integer)
            mpz_mod(list_elt.value, tmp, prime_pow.prime.value)
            ...
            mpz_divexact(tmp, tmp, prime_pow.prime.value)
            if neg:
                if mpz_cmp(tmp, prime_pow.pow_mpz_t_tmp(curpower)[0]) >= 0:
                    mpz_sub(tmp, tmp, prime_pow.pow_mpz_t_tmp(curpower)[0])
            PyList_Append(ans, list_elt)
        mpz_clear(halfp)
    mpz_clear(tmp)
    return ans
...
cdef int cteichmuller(mpz_t out, mpz_t value, long prec, PowComputer_class prime_pow) except -1:
    ...
    - ``out`` -- an ``mpz_t`` which is set to a `p-1` root of unity congruent to `value` mod `p`;
    ...
    mpz_init(u)
    mpz_init(xnew)
    # u = 1 / Mod(1 - p, prime_pow.pow_mpz_t_tmp(prec)[0])
    mpz_sub(u, prime_pow.pow_mpz_t_tmp(prec)[0], prime_pow.prime.value)
    mpz_add_ui(u, u, 1)
    mpz_invert(u, u, prime_pow.pow_mpz_t_tmp(prec)[0])
    # Consider x as Mod(value, prime_pow.pow_mpz_t_tmp(prec)[0])
    # xnew = x + u*(x^p - x)
    mpz_powm(xnew, out, prime_pow.prime.value, prime_pow.pow_mpz_t_tmp(prec)[0])
    mpz_sub(xnew, xnew, out)
    mpz_mul(xnew, xnew, u)
    mpz_add(xnew, xnew, out)
    mpz_mod(xnew, xnew, prime_pow.pow_mpz_t_tmp(prec)[0])
    # while x != xnew:
    #     x = xnew
    #     xnew = x + u*(x^p - x)
    while mpz_cmp(out, xnew) != 0:
        mpz_set(out, xnew)
        mpz_powm(xnew, out, prime_pow.prime.value, prime_pow.pow_mpz_t_tmp(prec)[0])
        mpz_sub(xnew, xnew, out)
        mpz_mul(xnew, xnew, u)
...
cdef inline int cconv_mpzt_out(mpz_t out, mpz_t x, long valshift, long prec, PowComputer_class prime_pow) except -1:
    """
...
cdef inline long cconv_mpqt(mpz_t out, mpq_t x, long prec, bint absolute, PowComputer_class prime_pow) except? -10000:
    """
